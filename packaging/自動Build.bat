...
if not "!DIRTY_SIZE!"=="0" (
    if "!STRICT!"=="1" (
        ...
        echo [WARN] 這種包只適合本機測試；正式交付請整理工作樹後改跑: packaging\自動Build.bat --strict
    )
)
...
:show_help
echo(Usage:
echo(  %~f0 [options] [build_release.py options]
echo(
echo(Examples:
echo(  %~f0
echo(      Local build + release gate + zip/sha256. Dirty tree adds --allow-dirty.
echo(
echo(  %~f0 --strict
echo(      Strict release mode. Dirty tree is not allowed.
echo(
echo(  %~f0 --skip-deps --no-cli-smoke
echo(      Faster build. Skip dependency install and packaged CLI smoke.
echo(
echo(Batch options:
echo(  --strict       Do not auto-add --allow-dirty.
echo(  --skip-deps    Skip requirements/PyInstaller install checks.
echo(  --no-pause     Do not pause at the end.
echo(  --help, -h     Show this help.
echo(
echo(Other args are forwarded to tools\build_release.py, for example:
