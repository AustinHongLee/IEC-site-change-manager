# -*- coding: utf-8 -*-
"""Generate a showcase from the project's real attachments folder."""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="用真實 attachments 建立資料展示輸出")
    ap.add_argument("--output", default=os.path.join(_ROOT, "staging", "real_attachments_showcase"), help="showcase 輸出資料夾")
    ...
    ap.add_argument("--overwrite", action="store_true", help="覆寫既有 showcase；只允許覆寫有 showcase marker 的資料夾")
...
def _run(args) -> dict:
    from real_attachments_showcase import run_real_attachments_showcase

    return run_real_attachments_showcase(
        args.output,
...
def _print_result(result: dict) -> None:
    print(f"Real attachments showcase：{'成功' if result.get('ok') else '失敗'}")
    print(f"showcase：{result.get('showcase', '')}")
